...
folder_comments = "../data/CommentsandRepliesLikedRelevance/"
	...
	for videoid in commentsandreplies:
		for commentid in commentsandreplies[str(videoid.encode("utf-8"))]:
			...
			try:
				...
				formal_score[ max( int( (fscore-1)/10 ) , 0) ] += (0.1+lCount * toConsider)
				inappropriate_score[ max( int( (iscore-1)/10 ) , 0) ] += (0.1+lCount * toConsider)
				sentiment_score[ max( int( (sscore-1)/10 ) , 0) ] += (0.1+lCount * toConsider)
...
if __name__ == '__main__':
	...
	folder_save = "./top650_modified/"
	Y = np.array(Y)
	X = np.array(X)
	np.save(folder_save + "Y-Vector",Y)
